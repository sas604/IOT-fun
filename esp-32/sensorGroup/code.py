...
while True:
    try:
        ...
        text = "\nTemperature: %0.1fC \nHumidity: %0.1f%% \nCO2:% s\nTube Hum:  %0.1f%%\nTube Hum:  %0.1f%%" % (
            sensor.temperature, sensor.relative_humidity, co2_ppm_level, relative_humidity, temperature)
        text_area = bitmap_label.Label(terminalio.FONT, text=text, scale=1)
        text_area.x = 0
        text_area.y = -15
        board.DISPLAY.show(text_area)
        mqtt_client.loop()
        mqtt_client.publish(mqtt_topic + '/mesurments', json.dumps({'temp': sensor.temperature, 'hum': sensor.relative_humidity,
                            'co': co2_ppm_level, 'tube_hum': relative_humidity,  'tube_temp': temperature}))
